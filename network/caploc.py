...
from network.backbone.cap import cap


class CAPLoc(nn.Module):
    """
            ConvNeXt-tiny + 2 ViP + CA
    """
    def __init__(self, feature_extractor, droprate=0.5, pretrained=True, feat_dim=384):
        super(CAPLoc, self).__init__()
        ...
        self.cap_block1.reset_classifier(num_classes=feat_dim)
        self.cap_block2.reset_classifier(num_classes=feat_dim)
        ...
        # initialize
        if pretrained:
            init_modules = [self.cap_block1, self.cap_block2, self.fc_xyz, self.fc_wpqr]
        else:
    ...
    def forward(self, x):
        ...
        xyz_block1 = self.cap_block1(x)
        wpqr_block2 = self.cap_block2(x)
